...
public class Launcher extends JFrame {
	...
	public void needDownload() {
		...
		progressBar.setValue(25);
		{
			...
			try {
				location = new URL("https://github.com/ENDPOINT-TO-RESOURCE");
				verfile = new URL("https://raw.githubusercontent.com/ENDPOINT-TO-RESOURCE/version.txt");
	...
	public Launcher() {
...

		{
			...
			try {
				...
				{
					URL url = new URL("https://raw.githubusercontent.com/ENDPOINT-TO-RESOURCE/dlls.txt");
					HttpURLConnection conn = (HttpURLConnection) url.openConnection();
					conn.setRequestMethod("HEAD");
					...
					if (!fi.exists() || fi.length() != conn.getContentLength()) {
						System.out.println("Downloading resource!");
						download("https://raw.githubusercontent.com/ENDPOINT-TO-RESOURCE/dlls.txt", "dlls.txt",
								conn.getContentLength());
					} else {
						System.out.println("No need to download resource!");
					}
...
//						progressBar.setValue(100);
				...
				for (final String str : downloadDLLs) {
					...
					URL url = new URL("https://github.com/ENDPOINT-TO-RESOURCE/" + str);
					final HttpURLConnection conn = (HttpURLConnection) url.openConnection();
					progressBar.setValue(30);
					conn.setRequestMethod("HEAD");
					...
					if (!fi.exists() || fi.length() != conn.getContentLength()) {
						...
						Thread t = new Thread() {
							@Override
							public void run() {
								download("https://github.com/ENDPOINT-TO-RESOURCE/" + str, "/" + str,
										conn.getContentLength());
							}
						};
						t.start();
		...
		try {
			URL url = new URL("https://raw.githubusercontent.com/ENDPOINT-TO-RESOURCE/YourGame64bit.jar");
			final HttpURLConnection conn = (HttpURLConnection) url.openConnection();
			conn.setRequestMethod("HEAD");
			if (needDownload || !f.exists() || f.length() != conn.getContentLength()) {
				...
				Thread t = new Thread() {
					...
					@Override
					public void run() {
						download("https://raw.githubusercontent.com/ENDPOINT-TO-RESOURCE/YourGame64bit",
								"YourGame64bit.jar", conn.getContentLength());
					}
				};
				t.start();
				threads.add(t);
		...
		File f2 = new File("YourGame32bit.jar");
		System.out.println("Exists: " + f2.exists());
		try {
			URL url = new URL("https://raw.githubusercontent.com/ENDPOINT-TO-RESOURCE/YourGame32bit");
			final HttpURLConnection conn = (HttpURLConnection) url.openConnection();
			conn.setRequestMethod("HEAD");
			if (needDownload || !f2.exists() || f2.length() != conn.getContentLength()) {
				...
				Thread t = new Thread() {
					...
					@Override
					public void run() {
						download("https://raw.githubusercontent.com/ENDPOINT-TO-RESOURCE/YourGame32bit",
								"YourGame32bit.jar", conn.getContentLength());
					}
				};
				t.start();
				threads.add(t);
		...
		String exec = (System.getProperty("user.dir") + java.io.File.separator + "YourGame64bit.jar");
